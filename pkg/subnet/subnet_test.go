...
func TestNewSubnet(t *testing.T) {
	for i := 1; i <= 32; i++ {
		...
		t.Log("hosts per network", s.NetworkHosts())
		t.Log("usable hosts per network", s.UsableNetworkHosts())
		t.Log("total hosts per subnet", s.NetworkHosts()*s.Networks())
...
func TestChildSubnets(t *testing.T) {
	...
	for _, item := range parentChildSet {
		...
		t.Log("hosts per network", subnet.NetworkHosts())
		t.Log("child subnet hosts per network", childSubnet.NetworkHosts())
