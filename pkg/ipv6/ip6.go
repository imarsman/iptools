...
func randomMacBytesForInterface(global bool) (bytes [6]byte, err error) {
	var mac [6]byte
	_, err = rand.Read(mac[:])
	if err != nil {
	...
	// Generally the first byte for a mac address is 0. This means that the 7th bit will be 0. The advice to set the
	// seventh bit is to invert it, since it will always be 0. However, with a randomly generated byte inverting the
	// seventh bit will randomly have it set to 0 or 1. Thus, here an argument is used that sets the 7th bit to zero or
	// one. Calls made from code always specify global == true.

	// It may be more correct to make the first byte 0 then set the 7th bit to 1 to match what mac addresses use.
	// However, I think that setting the 7th bit to 1 starting with a random value is OK.

	// fmt.Printf("%08b\n", mac[0])
	// If we want to use first byte of 0
	// mac[0] = 0x0
...
func randomGlobalUnicast() (addr netip.Addr, err error) {
	macAddrBytes, err := randomMacBytesForInterface(true)
	if err != nil {
		return
	}
